import actors.{Buyer, Seller, AuctionSearch}
...
class PersistentAuctionSpec extends TestKit(ActorSystem("AuctionSpec")) with ImplicitSender with WordSpecLike with BeforeAndAfterAll {
  ...
  "Auction System" must {
    ...
    "notify seller when auction had no winner" in {
      val aucSearch = system.actorOf(Props(classOf[PersistentAuctionSearch], List.empty[ActorRef]))
  ...
  "Auction search" must {
    ...
    "allow searching for auctions" in {
      ...
      val aucSearch = system.actorOf(Props(classOf[PersistentAuctionSearch], List(probe1.ref, probe2.ref)))
